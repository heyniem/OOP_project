...
public class Doll extends Enemy {
    ...
    private int temp;
    SmartAI dollAI = new SmartAI(false);
    //SuperDumbAI balloonAI = new SuperDumbAI();

    public Doll(int xUnit, int yUnit, Image img, int id) {
    ...
    public void update() {
        if (dead) {
            intervalDead++;
            if (intervalDead <= 11) setImg(doll_dead.getFxImage());
            else entities.remove(this);
        }
        else {
            if (x % SCALED_SIZE == 0 && y % SCALED_SIZE == 0) {
                direction = dollAI.chooseDirection(this.x, this.y, direction, 76);
            ...
            while (true) {
                ...
                if (direction == 2 || direction == 1) {
                    if (indexBalloon == 0) {
                        this.img = doll_right1.getFxImage();
                    }
                    if (indexBalloon == 1) {
                        this.img = doll_right2.getFxImage();
                    }
                    if (indexBalloon == 2) {
                        this.img = doll_right3.getFxImage();
                    }
                } else if (direction == 3 || direction == 0) {
                    if (indexBalloon == 0) {
                        this.img = doll_left1.getFxImage();
                    }
                    if (indexBalloon == 1) {
                        this.img = doll_left2.getFxImage();
                    }
                    if (indexBalloon == 2) {
                        this.img = doll_left3.getFxImage();
                    }
                }
                if (direction == 1) {
                    if (isFree(x + 2, y)) {
